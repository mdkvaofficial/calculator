    ...
    const handleAskLeave = async () => {
        try {
            ...
            if (user && companyId) {
                ...
                const q = query(leaveCollectionRef, where('userId', '==', user.uid), orderBy('date', 'desc'), limit(1));
            ...
            <View style={styles.recordContainer}>
                ...
                {item.status === 'pending' && (
                    <View style={styles.buttonContainer}>
                        <TouchableOpacity style={[styles.button, styles.approveButton]} onPress={() => handleGrantLeave(item.id)}>
                            <Text style={styles.buttonText}>Grant</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.button, styles.rejectButton]} onPress={() => handleRejectLeave(item.id)}>
