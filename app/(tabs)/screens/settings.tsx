...
import { Text, View, StyleSheet, TouchableOpacity, Alert, Modal, FlatList } from 'react-native';
import { NavigationProp } from '@react-navigation/native';
import { db, auth } from '../../../FirebaseConfig';
import { doc, deleteDoc, getDoc, updateDoc, query, where, getDocs, collection, writeBatch, orderBy, onSnapshot } from 'firebase/firestore';
...
const Settings: React.FC<SettingsProps> = ({ navigation }) => {
    ...
    const [leaveStatus, setLeaveStatus] = useState<Array<{ id: string; date: any; status: string }>>([]);
        ...
        try {
            ...
            if (userDoc.exists()) {
                ...
                if (companyId) {
                    ...
                    Alert.alert('Success', 'Company and associated users updated successfully', [
        ...
        const fetchLeaveStatus = async () => {
            try {
                ...
                if (user) {
                    ...
                    const q = query(leaveCollectionRef, where('userId', '==', user.uid), orderBy('date', 'desc'));
    ...
    const renderLeaveStatus = ({ item }: { item: { id: string; date: any; status: string } }) => {
    ...
    return (
        <View style={styles.container}>
            ...
            {userRole === 'Employer' && (
                <TouchableOpacity style={[styles.button, styles.deleteButton]} onPress={handleDeleteCompany}>
                    <Text style={styles.buttonText}>Delete Company</Text>
                </TouchableOpacity>
            )}

            {userRole === 'Employee' && (
                <TouchableOpacity style={[styles.button, styles.leaveButton]} onPress={handleLeaveCompany}>
